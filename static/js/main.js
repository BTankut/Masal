    ...
    function displayFavorites() {
        ...
        .then(serverFavorites => {
            ...
            taleFavorites.forEach((favori, index) => {
                ...
                <div class="history-item" data-id="${favori.id}" data-index="${index}">
                    <div class="history-thumbnail">
                        <img src="${favori.image_url ? ('/' + favori.image_url) : (favori.image || '/static/img/default-tale.jpg')}" alt="${favori.title || 'Favori Masal ' + (index + 1)}" class="history-image">
                    </div>
                    <div class="history-item-info">
                        <h4>${favori.title || 'Favori Masal ' + (index + 1)}</h4>
                        <p>${favori.characterName || ''} ${favori.characterType || ''}</p>
                ...
                if (taleHistory.length > 0) {
                    ...
                    if (historyContainer) {
                        ...
                        taleHistory.forEach((tale, index) => {
                            ...
                            <div class="history-item" data-id="${tale.id || ''}" data-index="${index}">
                                <div class="history-thumbnail">
                                    <img src="${tale.image_url ? ('/' + tale.image_url) : (tale.image || '/static/img/default-tale.jpg')}" alt="${tale.title || 'Masal ' + (index + 1)}" class="history-image">
                                </div>
                                <div class="history-item-info">
                                    <h4>${tale.title || 'Masal ' + (index + 1)}</h4>
                                    <p>${tale.characterName || ''} ${tale.characterType || ''}</p>
    ...
    function updateTaleFavoritesUI() {
        ...
        taleFavorites.forEach((tale, index) => {
            favoritesHTML += `
                <div class="history-item" data-id="${tale.id || ''}" data-index="${index}">
                    <div class="history-thumbnail">
                        <img src="${tale.image_url ? ('/' + tale.image_url) : (tale.image || '/static/img/default-tale.jpg')}" alt="${tale.title || 'Favori Masal ' + (index + 1)}" class="history-image">
                    </div>
                    <div class="history-item-info">
